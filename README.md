...
**Knowledge Graph & Codebase Visualization.** orangu incrementally parses your entire codebase offline using Tree-sitter, mapping every function, class, and call into a dependency graph. The AI uses this graph to instantly pinpoint the most central code symbols for any query without flooding its context window. Use `/graph explain <symbol>` to inspect a node's source, degree, community, and evidence-tagged relationships, or `/graph path <source> <target>` to trace a shortest relationship path. Pass either command to `orangu -p` for a one-shot query; bare `/graph` generates an interactive HTML visualization of the codebase architecture that opens in any browser.
