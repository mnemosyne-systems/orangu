...
| Command | What it does |
| --- | --- |
| `orangu loop --turns 3 -- 'Fix the parser'` | Repeat coding and read-only review. `--time 30m` or `--until 'condition'` are the other stop policies. |
| `--file loop.yaml` `status` `pause` `resume` `clear` | Load a YAML definition; inspect, safely pause, continue or cancel the saved workspace loop. |
