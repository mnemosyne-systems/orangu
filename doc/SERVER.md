...
- `threads` — CPU worker threads; unset, one per core for loading, and requests
  run inside worker pools chosen per phase by measurement (every core, or the big
  ones on a big/little machine — `[adapt] cpu pool` lines, `ORANGU_CPU_POOLS=0`
  for the one pool). Setting it pins that one pool size.
