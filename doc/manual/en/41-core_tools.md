...
The report ends with the **Conclusion**, derived from the file statuses rather than from the model: the verdict — `orangu approves this patch` when every file is approved, or `orangu rejects this patch` when any file was rejected or not reviewed — stands alone in bold rather than as a list item, followed by the reviewing model's name in parentheses (not bold), e.g. `**orangu approves this patch** (gemma)`; the affected files then follow as a bullet list in bold, grouped by their status (`Rejected: **file**`, `Not reviewed: **file**`).
