...
- `llama.rs` — grouped-query attention, RoPE, RMSNorm, SwiGLU: the shape
  ...
  table for `LLM_ARCH_LLAMA`), and `qwen3moe` — the same block with a
  mixture in place of the dense feed-forward. A layer's feed-forward is
  whichever the file carries: the router tensor (`ffn_gate_inp`) is what
  says a layer routes, and a mixture layer takes the step-by-step path
  because the chains that fuse a feed-forward onto the device compute a
  dense network and have nowhere to put a per-token expert choice.
...
| :-- | :-- | :-- |
| `ORANGU_PACKED_GEMM` | off (`1` turns it on) | Runs the **float** prefill matmuls (`F32`, `F16`, `BF16` weights) through a packed, cache-blocked GEMM from the `rten-gemm` crate instead of this engine's own tiled kernel. The switch is read where the kernel is *chosen*, not inside it, so the shape rules can be tested independently of the default. **Off because it is not faster on the hardware it was measured on.** With normal float values the two kernels are level — between 0.92x and 1.28x across every shape a forward pass runs, and the spread between repeats of one shape is wider than the difference between the kernels at that shape; end to end on a mixture prefill it is worth about 1% at a 624-token prompt and nothing at 2,236, where the closing control of an interleaved A/B came back at the treatment's rate. The packed kernel's advantage depends on the host's instruction set, and the machine this was measured on has `AVX2` but neither `AVX-512` nor `VNNI`, so the crate selects its weakest x86 kernel — the comparison is worth repeating where it can select its best, which is what this knob is for. It never applies to quantized weights: this engine multiplies the packed bytes directly, and dequantizing them so a float GEMM can read them costs more than the whole product. It never applies at one token either, where a mat-vec has no reuse to block for. |
| `ORANGU_PREFILL_BATCH` | unset (chosen per prefill; see below) (integer, not a presence flag) | How many prompt tokens go into one forward pass. Setting it fixes the width in every regime and overrides the choice described here; `0` means the whole prompt in one pass. **Left unset, the width depends on the backend and on how much of the model is in RAM.** On a backend with a driver submission timeout the value is a *ceiling* the adaptive sizer works below (see `ORANGU_PREFILL_CHUNK_MS`), and it is `512`: measured on a 4 GiB card holding 2.5 GiB of weights with a 17.5k-token prompt, no chunking lost the device after 21 s, `2048` after 3m54s, and `512` completed in 4m13s with peak VRAM identical (3.67 GiB) in all three. On a backend with no such timeout the width is flat — no probe, no shrink — and is `512` when the model is resident and the whole prompt when it is not. Resident, narrow is faster and smaller: at 8,001 tokens, three interleaved pairs, `512` prefilled **10.6% faster** than one pass and peaked at 1,988 MB against 2,967. Streamed from disk the ordering reverses, because every extra pass re-reads what is not cached: at ~1,030 tokens one pass ran at **45.7 tok/s reading 1.29 GiB**, against 35.4 tok/s and 4.97 GiB at width 256. **A mixture-of-experts file whose routed experts run on the card gets `2048` instead of `512`**, for the same reason one level down the memory hierarchy: each pass streams a layer's whole expert stack across the bus, so a prompt taken in five passes moves five stacks. Measured on a 30B-A3B file at a 2,236-token prompt — `512`: 86.5 tok/s moving 0.34 MiB of expert weight per token; `1024`: 94.7 and 0.23; `2048`: 96.3 and 0.17. **A file with a sliding window is excluded**, because its ring mirrors are sized as the window plus one chunk: widening the chunk doubles them, and on a 26B-A4B file that cost 18.7% at 626 tokens and 32.5% at 2,238 even though the expert traffic halved. |
| `ORANGU_PREFILL_CHUNK_FLOOR` | unset (integer; `0` is no floor) | The narrowest chunk the adaptive sizer may choose, overriding the floor the model itself asks for. Two kinds of model ask for one. A model with layers on the host is floored at `ORANGU_HOST_STREAM_TOKENS`, below which a host layer's projections run on the CPU rather than streaming to the card — a 16-token probe measured there says a 512-token chunk would take minutes, and every chunk after it stays narrow. A mixture whose routed experts run on the card is floored at `ORANGU_EXPERT_GEMM_TOKENS` for the same reason: below it the chunk takes the host's per-expert path, which is slower, which sizes the next one narrower still. A model with both reasons gets the larger of them. What the floor does in practice is make the opening probe a real pass: measured on a 30B-A3B file, **+6.7%** at a 2,236-token prompt (two expert-stack streams rather than three) and −2.7% at 624 (where the 16-token probe on the host path was the cheaper opening); on a 26B-A4B file, level at both. It exists as a knob so both settings can be compared on one binary — a floor is chosen by what the model is, so otherwise the two arms would be two builds. |
...
| `ORANGU_Q4K_I8` | on where `ORANGU_DECODE_MMVQ` is (`0` turns it off) | `Q4_K`'s decode mat-vec on the integer dot (`vulkan_shaders::shader_source_q4k_i8`), for projections of 512 rows or more: the fused chain's 8-bit activation (the same the types above read) against the weight nibbles through `dot4I8Packed`, each super-block's 6-bit scales and minimums applied once to the integer sums — on the skeleton of the ternary integer-dot kernel, 64 lanes, eight to a super-block, eight super-blocks in flight, the activation words loaded once and reused by every row of the workgroup. Measured on the CIX P1's Mali-G720 with `gemma-4-E2B` at `Q4_K_M`: `ffn_gate` and `ffn_up` 368 → 265 µs a layer, `ffn_down` 321 → 284, `attn_q` 100 → 88; narrower projections (the 256-row K and V) measured slower on it and keep the float kernel. Checked at start-up against the float kernel at the 8-bit rounding bound (`decode_kernel_agrees`), as the other integer-dot kernels are. `0` is the control arm. |
| `ORANGU_Q4K_I8_ROWS` | `4` (`1..=16`) | Rows per run of `ORANGU_Q4K_I8`'s kernel: the activation words of a block are loaded once and reused by every row. |
| `ORANGU_Q4K_I8_GROUPS` | `4` (`1..=16`) | Runs of `ORANGU_Q4K_I8_ROWS` rows one workgroup takes in sequence. Swept on the Mali-G720 for `ffn_gate`: 4 × 4 265 µs, 4 × 8 274, 8 × 4 292, 8 × 2 308, 2 × 8 303, 8 × 1 399, 16 × 1 453, 1 × 16 453. |
...
| `ORANGU_HYBRID_PREFILL_CPU` | unset (`1` turns it on) | `[orangu-server].prefill_backend = cpu` from the environment (it overrides `device` and `auto`): the Gemma and Qwen hybrid trunks run a multi-token pass on the CPU backend and single-token decode on the selected device. On the CIX P1 with the 27B ternary model: prefill 1.6 → 10.3 tok/s at 64 tokens (11.3 at 256), decode unchanged. The attention caches and the recurrent state move between the two through the accessors that already keep one copy current. |
| `ORANGU_DECODE_PROBE` | on (`0` turns it off) | Under `backend = auto` on one GPU, time a few one-token decode steps of the model on the device and on the CPU backend (the same mapped weights) at load, and decode on the cores only when they are at least 1.1× faster (`engine::decode_backend`). `0` keeps the device without measuring, as before. |
| `ORANGU_DECODE_PROBE_MIN_GAIN` | `1.1` (a positive number) | How much faster the cores must decode for `ORANGU_DECODE_PROBE` to move decode to them. A measurement knob: a value below 1 makes a slower CPU win, which is how the switch itself is exercised on a machine whose GPU is the faster (on the CIX P1, `0.1` served the model from the CPU at `backend = cpu`'s 8.3 tok/s). |
| `ORANGU_HEAD_SPLIT` | on (`0` turns it off) | When decode moved to the cores and left a GPU idle, measure sharing the output head's rows with a fresh backend on that GPU, and keep the split only if whole decode steps get ≥ 1.05× faster (`engine::head_split`). `0` keeps the head on the cores and skips the ~1 s measurement. |
| `ORANGU_HEAD_SPLIT_SHARE` | unset (a number in `0..1`) | The device's share of the head's rows for `ORANGU_HEAD_SPLIT`, kept whatever the measurement says. A measurement knob. |
| `ORANGU_SDOT_K_ROWS` | on (`0` turns it off) | On aarch64 with `sdot`, decode's one-token `Q4_K`/`Q6_K` row dot takes the super-block-wide kernels (`vecdot::dot_k_row_q4_k_sdot`/`_q6_k_sdot`; bit-identical, ~1.5× per core). `0` keeps the per-32 form, for measuring (`doc/PERF-ALL.md`, task 15). |
| `ORANGU_CPU_POOLS` | on (`0` turns it off) | With the default worker pool, the pools a request runs *inside* (`engine::cpu_pools`): every core, and on a machine with big and little cores the big ones, pinned — each CPU phase timed in both after load (prompts on the model's widest FFN GEMM at 256 tokens, decode on its one-token steps) and run in the faster. Running inside a pool matters by itself: a request on a tokio thread hands each parallel region to the global pool from outside, and on the CIX P1 decode on the same eight cores went 12.6 → 17.5 tok/s from that alone; on the big cores rather than all twelve, 8.2 → 17.7. `0` runs requests on the global pool, as before. Not built when `threads` or `ORANGU_EXPERT_BIG_CORES` names the pool. |
| `ORANGU_ATTENTION_MIXED` | on where the CPU has `i8mm` and `bf16` (`0` turns it off) | A prompt's attention on the CPU backend (a pass of at least `ORANGU_ATTENTION_TILED_MIN_TOKENS` tokens) in blocks of 32 rows — query tokens times the heads of one K/V group — with the scores in `int8` on `smmla` (keys less their per-channel mean, quantized per row: the SageAttention recipe the picture transformers use) and the value product in `bf16` on `bfmmla`, each block against only the keys its windows cover. Measured on the CIX P1 at `gemma-4-E2B`'s shapes over 2048 tokens: 5.4× the one-query loop on the full-attention layers, 3.9× on the sliding ones; prompts 127 → 154 tok/s at 2048 tokens and 115 → 147 at 4096. Within `int8`/`bf16` rounding of the `f32` loop, not bit-identical to it. `0` falls back to `ORANGU_ATTENTION_TILED`'s `f32` tiles. |
| `ORANGU_ATTENTION_TILED` | on (`0` turns it off) | Where `ORANGU_ATTENTION_MIXED` cannot run (no `i8mm`/`bf16`, or turned off): a prompt's CPU attention in `f32` tiles — a block of queries and their K/V group against a block of keys, online softmax. Within float reassociation of the one-query loop; about 1.06× it on the CIX P1, where that loop was already compute-bound. `0` leaves the one-query loop for every pass. |
| `ORANGU_ATTENTION_TILED_MIN_TOKENS` | `32` (integer) | The narrowest pass the two blocked attention kernels above take; narrower passes, and decode, keep the one-query loop, which spreads over the pool by token. |
| `ORANGU_PROMPT_WEIGHTS` | unset (`[orangu-server].prompt_weights`, `auto`) | `auto`, `copy` or `file` over the configuration for one run: whether the weights a prompt multiplies on the CPU run from copies for the cores' matrix instructions — per-row `int8` for the K-quants (`vecdot::RowI8`, the `smmla` tile), `bf16` for the unquantized matrices (`vecdot::Bf16Weights`, the `bfmmla` tile) — or from the file's weights. `auto` measures each kind's widest matrix both ways at load and needs a 10% gain and the memory; see `prompt_weights` in the server configuration. |
| `ORANGU_PREFIX_WARMUP` | unset (`[orangu-server].prefix_warmup`, on) | `0`/`off` turns off, anything else on, over the configuration for one run: the record of reused prompt prefixes and their background prefill after a restart. The `[adapt] prefix warm-up:` line says what was warmed and how long it took. |
...
| `ORANGU_BUSY_POLL` | `1` (spin; `0` block, `auto` predicted) | How the decode thread passes the wait for a token's readback. `0` blocks on the fence for the whole wait: the thread is parked while the card runs, its core drops into a deep idle state (hundreds of microseconds to leave on a laptop part), and the fence's completion reaches it through an interrupt and a scheduler wake-up — so the next token's recording starts late, and the card idles between tokens long enough for its clock governor to settle below its top state. `1`, the default, spins on a non-blocking poll for the whole wait: back within microseconds of the card finishing, the core hot, at the price of one core busy for the whole wait. `auto` blocks for most of the wait as the last waits of the same kind measured it and spins through the rest (`ORANGU_BUSY_POLL_BLOCK`, the blocked share in percent, default `75`) — measured at about half the spin's gain for a quarter of its core time, since the core kept awake through the wait is most of the effect. Measured on real `RX 5500M` at depth 0, the spin against the block: SmolLM2-360M 131 → 171 tok/s, Llama-3.2-1B 125 → 132, 3B 58 → 60, E2B 72 → 74; level on the 26B-A4B and Nemotron, whose experts run on the host. `0` is the choice for a machine that serves other work beside the model. `ORANGU_WAIT_TRACE=1` prints every 32nd wait: its length, the prediction, the blocked part and the polls spun. |
...
| `ORANGU_ATTN_SPLIT_K` | `8` for the classic kernel, `32` for the cooperative one, `64` for the cooperative one at a 16-lane subgroup (a power of two, `1..=64`) | Split-k factor for decode attention: how many workgroups each query head's KV-position range is split across (`n_head × k_num` phase-1 workgroups, merged by a phase-2 pass). Each head attends a KV range that *grows with context*, so more splits expose more parallelism the longer a generation runs, at the cost of more phase-2 merge overhead when the range is short. Raised from `4` to `8` after re-sweeping in the full decode chain at real context lengths (the earlier sweep was on the isolated dispatch at short context): on real `E2B`/`RX 5500M`, `k_num=8` cut per-token GPU time at ~245 tokens of context from 35.6 to 32.4 ms while staying neutral at short context, and had the best end-to-end throughput of `{4,8,16}`; `16` wins only once context is very long. Byte-identical output across values. Workloads dominated by very long contexts may prefer `16`. Pure runtime uniform — no shader rebuild. 64 is a hard ceiling: the wide split merge holds a split's running max and sum in 64-entry workgroup arrays, and at 128 every head past the first read another's partials (NaN). |
| `ORANGU_ATTN_COOP16` | on where the device's subgroups are narrower than 32 (`0` turns it off) | Decode attention on the cooperative kernel (`shader_source_attention_split_coop`) at the subgroup's own width: a workgroup of one subgroup, each lane owning every 16th dim of the head so K and V are read coalesced and the score is one `subgroupAdd` — where the 32-lane cooperative kernels are not built (the Mali-G720's subgroups are 16 wide) and decode fell back to the classic split kernel's serial per-thread dot. Measured on the CIX P1 with `gemma-4-E2B`: decode attention 4.4 → 1.1–1.8 ms a token at depth 0 and 27.8 → 7.3 at depth 2048. Prefill keeps its own kernels. `0` is the control arm. |
...
| `ORANGU_DECODE_CHUNKS` | unset (integer; set, `3` equal chunks) | How many `queue.submit()` calls one decode step's layer loop is split across. `1` records the whole token and submits once (the historical behaviour); `> 1` submits the first `chunks - 1` groups of layers as soon as they are recorded, so the GPU starts executing them while the CPU is still recording and validating the later ones — overlapping the CPU-side submission cost with GPU execution instead of serialising it. Clamped to `1..=n_layers`. On real `E2B`/`RX 5500M` this raised decode throughput from 14.4 tok/s (`1`) to 18.8 tok/s (`7`, the default), with byte-identical output; `35` (one submit per layer) reaches 19.4 tok/s but adds per-submission overhead for a marginal gain. **Unset, the split is not equal:** the plan is the probed geometric split `ORANGU_DECODE_FIRST_CHUNK` describes; setting this pins equal chunks. |
| `ORANGU_DECODE_FIRST_CHUNK` | probed (integer; `0` = equal chunks) | Layers in a decode step's *first* submission; each following chunk is `ORANGU_DECODE_CHUNK_RATIO` times the last. Only the recording ahead of the first submission is host time the device cannot hide, so the default plan carries two layers first and grows each chunk threefold so the submissions stay few. Two rather than one: a single-layer first burst after a pause, when the card has parked its clocks, was measured holding a whole request at a third of its rate; two layers never did. The split applies to every family's fused decode chain (the llama family, `phi3`, `mistral3`, `muse-glimmer` and gemma). **Unset, the plan is probed**: the server times its decode steps on three plans — the default, the same with each chunk twice the last, and equal chunks of four layers — over three rounds of a block of steps per plan (after 32 settling steps), and keeps the plan that won most rounds by 5%; the first answer is asked again after 1024 steps, later ones every 4096. The plans differ per model with no rule read off the file: a 360M model ran +11% on the twofold plan and +20% in fours, a 2B +13% twofold, a 1B −7% twofold. Setting this, `ORANGU_DECODE_CHUNKS` or `ORANGU_DECODE_CHUNK_RATIO` pins the plan and stops the probe. `ORANGU_CHUNK_TRACE=1` prints each round's medians and the vote. |
| `ORANGU_DECODE_CHUNK_RATIO` | probed (integer ≥ 1) | The growth of the decode step's submissions after the first: with a first chunk of two layers, `3` submits a 32-layer token as 2, 6, 18 and 6 layers; `2` as 2, 4, 8, 16 and 2. A larger ratio submits fewer times, a smaller one keeps the device closer behind the host's recording. Setting it pins the plan (see `ORANGU_DECODE_FIRST_CHUNK`). |
...
| `ORANGU_EXPERT_REGION_RELEASE` | off (`1` turns it on) | Gives the expert streaming region back to the card after each prompt's prefill and takes it again at the next prompt that needs it. Off now that the KV pool is sized from the driver's budget with the region already left out: holding the region costs the pool nothing, while releasing it made its return depend on what the card had left — less than the region plus its headroom once the pool held its share, so from the second prompt on the routed experts ran on the host. The return is judged against the driver's budget rather than the card's raw used bytes, which count other tenants the budget excludes. Taking it again is refused, and that prompt's routed experts run on the host, when the card has less left than the region plus `ORANGU_EXPERT_REGION_HEADROOM_MIB` (256): a region created into a full card lands in host memory and reads across the bus at every dispatch, and what it displaces reads that way for the rest of the process — a card filled to its last page stops moving anything back. Measured on a 26B-A4B file on a 4 GiB card: with the region held across prompts, a deep prompt after a shallow one decoded at 7.3 tok/s for the rest of the process against 10.7 with it released. `0` holds the region for the process, the control arm. |
...
| `ORANGU_MMQ_ROWS` | `0` (the rule; `64` or `128` fixes it) | The **row** tile of the integer-dot prefill GEMM, the twin of `ORANGU_MMQ_TOKS`: the two together pin the whole tile, so a shape's geometry can be swept in situ rather than only in the kernel probe. A shape whose output width is not a multiple of the pinned tile falls through to the narrow kernel as an unpinned call would. Measured on `gemma-4-E2B-it` `Q4_1`, the 128-row tile is 1.6× slower than the 64-row one on the down projection at 512 tokens, which is the rule's own answer. |
| `ORANGU_MMQ_TRACE` | unset (off) | Prints one line per distinct shape the integer-dot prefill GEMM serves — weight type, `[in_dim × out_dim]`, batch width, the kernel chosen and the dispatch grid — the first time each is dispatched. Answers "which kernel is this op actually running", which a device timestamp on its own cannot. |
...
| `ORANGU_TAIL_HOST` | probed (`0` device, `1` host) | Whether the vocabulary projection runs on the host after an `n_embd`-sized readback or on the card before an `n_vocab`-sized one. Unset, the server times the two: after 32 settling steps, three rounds each of a block of consecutive decode steps on the device arm then a block on the host arm; the host wins a round only when its median step is 10% faster, and takes the tail only when it wins most rounds. The first answer is asked again after 1024 steps, later ones every 4096. The blocks matter: arms alternating step by step let a host step park the card's clock and the device step after it was timed at the parked clock, so the host won by its own presence. The settling and the vote matter too: one pair of blocks timed on a process's first tokens saw the device arm at twice its settled step and lost to the host one start in six, and that process then ran 40% behind for its whole window. `ORANGU_TAIL_TRACE=1` prints each round and the vote. |
...
| `ORANGU_NPU_FFN` | measured (`0` never, `1` always) | Whether a prompt's feed-forward blocks go to the NPU's precompiled graphs. Unset, it is measured once the CPU prompt path is final (prompts on the CPU, `prompt_weights` prepared): one block is timed on the NPU at the width its graphs were compiled for and through the CPU's own prompt path at 384 tokens — the width it takes a prompt in — and the NPU is used only when it is at least 1.25× faster a token, since it also holds every chunk of the pass to its width and leaves the cores idle while it runs. The `[adapt] npu:` line says what was measured. The first form of the check compared both sides at the NPU's 128 tokens and flipped between starts on the CIX P1 (11.7 ms against 16.4 in one); the start that took the NPU prefilled `gemma-4-E2B` at 74 tok/s where the cores alone do 150. When prompts run on a device the NPU is kept. `0` turns the NPU's feed-forward blocks off, `1` uses them without measuring. |
...
| `ORANGU_DUMP_SHADERS` | unset (a directory path) | Set to a directory to write the generated WGSL of the decode-path kernels (Q4_K/Q6_K matmul-vec, RMSNorm, split-attention), the tiled prefill GEMM and the integer-dot prefill GEMM (per weight type and row tile) into it as `.wgsl` files at startup, then continue normally. A profiling aid: pair it with the driver's own `RADV_DEBUG=shaders,shaderstats` (which dumps the compiled ACO ISA and register/occupancy stats for this GPU), or hand the WGSL to an offline analyzer such as Radeon GPU Analyzer on an RDNA3+ machine. No effect on the computation. |
...
| `ORANGU_TEST_QWEN3MOE_MODEL` | `qwen3moe` routed-feed-forward forward-pass test | A local Qwen3-MoE `.gguf` file (e.g. Qwen3-Coder-30B-A3B-Instruct) |
