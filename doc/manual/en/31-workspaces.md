...
`orangu-server` has the same concept and the same flag: `-w <path>` or `--workspace <path>` sets the root directory the server operates in, defaulting to the directory it was started in. See the Inference server chapter.
